...
def dashboard(df):
    ...
    with top_cols[1]:
        ...
        pred_em_fila = int((df_f["Qtd Preditivas Atrasadas"] > 0).sum()) if "Qtd Preditivas Atrasadas" in df_f.columns else 0
        pred_em_dia = max(total_ativos - pred_em_fila, 0)

        prev_em_fila = int((df_f["Status Preventiva"] == "ATRASADA").sum()) if "Status Preventiva" in df_f.columns else 0
        prev_em_dia = max(total_ativos - prev_em_fila, 0)

        pred_data = pd.DataFrame({
            "Status": ["Preditivas em Fila", "Preditivas em Dia"],
            "Quantidade": [pred_em_fila, pred_em_dia],
        })
        prev_data = pd.DataFrame({
            "Status": ["Preventivas em Fila", "Preventivas em Dia"],
            "Quantidade": [prev_em_fila, prev_em_dia],
        })
    ...
    preventivas_em_fila = int((df_f["Status Preventiva"] == "ATRASADA").sum()) if "Status Preventiva" in df_f.columns else 0
    preditivas_em_fila = int((df_f["Qtd Preditivas Atrasadas"] > 0).sum()) if "Qtd Preditivas Atrasadas" in df_f.columns else 0
