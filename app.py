...
def dashboard(df):
    ...
    st.subheader("📈 Progresso atual x meta das preventivas")
    st.caption("Visual de acompanhamento acumulado com a linha do ritmo atual, a linha da meta e a linha do total a concluir.")

    pendentes = int((df_f["Preventiva Concluída"] != "SIM").sum()) if "Preventiva Concluída" in df_f.columns else 0
    ...
    if pendentes == 0:
        ...
        total_pendentes = [pendentes for _ in range(horizonte + 1)]
        ...
        fig_proj.add_trace(go.Scatter(
            ...
            y=total_pendentes,
            mode="lines",
            name="Total a Concluir",
            line=dict(color="#7f7f7f", width=2, dash="dash"),
            hovertemplate="%{x|%d/%m/%Y}<br>Total a Concluir: %{y}<extra></extra>",
        ))
        ...
        fig_proj.update_layout(
            ...
            yaxis_title="Preventivas concluídas (acumulado)",
        ...
        c_meta, c_atual, c_pendentes = st.columns(3)
        ...
        c_pendentes.metric("Preventivas pendentes hoje", f"{pendentes}")

        st.info(
            f"""
            🔹 **Leitura do gráfico:** a linha azul mostra a **progressão atual acumulada**, a linha verde mostra a **meta acumulada**, e a linha cinza mostra o **total que precisa ser concluído**.  
...
def pagina_ajuda():
    ...
    st.markdown(
        ...
        **Indicadores do dashboard**
        ...
        - **Gráfico de progresso x meta**: exibe em linhas a progressão atual acumulada, a linha da meta e o total a concluir com datas estimadas para cada cenário.
        """
    )
